...
//!
//! Scan data available from ibeo LUX laserscanners (not available for ibeo LUX prototypes).
//! Each scan data block starts with a header followed by the scan point list.
//! The data is encoded in little endian format!
//!
//! For angle information the unit angle ticks is used. An ibeo LUX typically uses 11520 ticks per rotation
...
class Scan2202 : public SpecializedDataContainer
{
...
public:
    ...
    enum class Flags : uint16_t
    {
        ...
        RearMirrorSide = 0x0400U //!< \b 0 = front, \b 1 = rear
...
public:
    constexpr static const char* const containerType{"ibeo.sdk.specialcontainer.scan2202"};
...
public: // setter
    ...
    void setScanPoint(const unsigned int idx, const ScanPointIn2202& sp) { this->m_points[idx] = sp; }
...
protected:
    static const uint16_t nbOfAngleTicksPerRotation{11520};

protected:
    static constexpr const char* loggerId = "ibeo::common::sdk::Scan2202";
    static ibeo::common::logging::LoggerSPtr logger;

protected:
    uint16_t m_scanNumber{0};
    uint16_t m_scannerStatus{0}; //as recieved from FPGA (struct above)
    uint16_t m_syncPhaseOffset{
        0}; //Phase difference (conversion factor 409.6 ns) bwn. sync sig and mirror cross sync angle
    NTPTime m_startNTP{0};
    NTPTime m_endNTP{0};
    uint16_t m_angleTicksPerRotation{0}; //counter ticks per rotation (for conv. angle ticks to rad or deg)
    int16_t m_startAngleTicks{0}; // [ticks]
    int16_t m_endAngleTicks{0}; // [ticks]
