...
//!$$IBEO_LICENSE_END$$
//!
//!\refactoring_author Mohamed Riad (mor)
//!\refactoring_date March 13, 2018
//------------------------------------------------------------------------------
...
#include <ibeo/common/sdk/datablocks/TimeRelationBase.hpp>
...
/**
 * TimeRelation stores information relating the reference timebase to an
...
public:
    ...
    {
        ibeo::common::sdk::timerelation::OtherTime m_standardError;
        ibeo::common::sdk::timerelation::OtherTime m_maxDeviation;
    ...
    struct Entry
    {
        ...
        ibeo::common::sdk::timerelation::OtherTime interpolate(ibeo::common::sdk::timerelation::RefTime const& r) const;
        ibeo::common::sdk::timerelation::RefTime interpolate(ibeo::common::sdk::timerelation::OtherTime const& o) const;

    public:
        ibeo::common::sdk::timerelation::RefTime m_refTime;
        ibeo::common::sdk::timerelation::OtherTime m_otherTime;
        double m_factor;
...
public: // Accessors
    ...
    // ----------------------------------------
    ibeo::common::sdk::timerelation::RefTime minRefTime() const;
    ...
    //!  \sa getRefTimeRanges(), getOtherTimeRanges()
    // ----------------------------------------
    ibeo::common::sdk::timerelation::RefTime maxRefTime() const;
    ...
    //!  \sa getRefTimeRanges(), getOtherTimeRanges()
    // ----------------------------------------
    ibeo::common::sdk::timerelation::OtherTime minOtherTime() const;
    ...
    //!  \sa getRefTimeRanges(), getOtherTimeRanges()
    // ----------------------------------------
    ibeo::common::sdk::timerelation::OtherTime maxOtherTime() const;
    ...
    //!  vector.
    // ----------------------------------------
    const ibeo::common::sdk::timerelation::RefTimeRangeVector& getRefTimeRanges() const;
    ...
    // ----------------------------------------
    const ibeo::common::sdk::timerelation::OtherTimeRangeVector& getOtherTimeRanges() const;
    ...
    //!  or inside a gap.
    // ----------------------------------------
    const ibeo::common::sdk::timerelation::RefTimeRange&
    getUnambiguousRange(ibeo::common::sdk::timerelation::RefTime const& t) const;
    ...
    // ----------------------------------------
    const ibeo::common::sdk::timerelation::RefTimeRange&
    getUnambiguousRangeNoThrow(ibeo::common::sdk::timerelation::RefTime const& t) const;
    ...
    // ----------------------------------------
    ibeo::common::sdk::timerelation::OtherTime convert(ibeo::common::sdk::timerelation::RefTime const& t) const;
    ...
    //!  \throws Never
    // ----------------------------------------
    ibeo::common::sdk::timerelation::OtherTime convertNoThrow(ibeo::common::sdk::timerelation::RefTime const& t) const;
    ...
    // ----------------------------------------
    ibeo::common::sdk::timerelation::RefTime convert(ibeo::common::sdk::timerelation::OtherTime const& t) const;
    ...
    //!  \throws Never
    // ----------------------------------------
    ibeo::common::sdk::timerelation::RefTime convertNoThrow(ibeo::common::sdk::timerelation::OtherTime const& t) const;
    ...
    //!  \throws Never
    // ----------------------------------------
    ibeo::common::sdk::timerelation::RefTimeVector
    convertAmbiguous(ibeo::common::sdk::timerelation::OtherTime const& t) const;

    static double calculateFactor(const ibeo::common::sdk::timerelation::RefTime& r1,
                                  const ibeo::common::sdk::timerelation::RefTime& r2,
                                  const ibeo::common::sdk::timerelation::OtherTime& o1,
                                  const ibeo::common::sdk::timerelation::OtherTime& o2);
...
private:
    ...
    EntryList::const_iterator findInterpolationEntry(const ibeo::common::sdk::timerelation::RefTime& r) const;
...
private: // Metadata
    struct Metadata
    {
        ...
        std::size_t findRangeContaining(const ibeo::common::sdk::timerelation::RefTime& t) const;
    ...
    public:
        ...
        ibeo::common::sdk::timerelation::RefTimeRange m_refTimeRange;

        // True if there is a gap somewhere in the OtherTimes
        bool m_hasOtherGap;
        // Minimum and maximum valid OtherTimes
        ibeo::common::sdk::timerelation::OtherTimeRange m_otherTimeRange;

        // A vector of the valid time-ranges (can each include multiple entries)
        ibeo::common::sdk::timerelation::RefTimeRangeVector m_refRanges;
        ibeo::common::sdk::timerelation::OtherTimeRangeVector m_otherRanges;
...
private:
    struct CompRef
    {
        bool operator()(const Entry& e, ibeo::common::sdk::timerelation::RefTime const& t) const
        {
            return e.m_refTime < t;
        }

        bool operator()(ibeo::common::sdk::timerelation::RefTime const& t, const Entry& e) const
        {
            return operator()(e, t);
        }
    };
