...
//!
//! The vehicle state is calculated by ibeoLUX from received CAN-Data.
...
//!
//! General data type: \ref ibeo::common::sdk::VehicleState
class VehicleState2805 : public SpecializedDataContainer
{
...
public:
    enum class ErrorFlags : uint16_t
    {
        ...
        LastFwaNotUpToDate
        = 0x0200U, ///< There is no latest (current) measurement of front wheel angle, or could not be calculated by SWA
        BufferEmpty = 0x0800U ///< No CAN data received
    }; // ErrorFlags

public:
    constexpr static const char* const containerType{"ibeo.sdk.specialcontainer.vehiclestate2805"};

    static constexpr uint64_t getClassIdHashStatic() { return hash(containerType); }

public:
    VehicleState2805();
    virtual ~VehicleState2805();
...
protected:
    ...
    uint16_t m_scanNumber{0}; //!< scan number of the scan this vehicle state is associated with.
    ...
    uint16_t m_calculationMethod{0}; //!< calculation method for motion estimation:
        //!< 0=unknown, bit1=SteeringWheelAngleUsed, bit2=YawRateUsed
        // (was reserved1)
    //========================================
    //! \brief Current yaw rate [0.0001rad] from latest CAN-Message. Since 2.5.00.
    //! \remark Available since firmware version 2.5.00.
    //-------------------------------------
    int16_t m_currentYawRate{
        0}; //!Yaw rate from latest CAN-Message received. Available since firmware version 2.5.00. [0.0001 rad/s]
