...
ibeo::common::logging::LoggerSPtr MsgBufferBase::logger{
...
{
    recvDataHeader = nullptr;
    startOfMsgBody = nullptr;
    ...
    {
        if (!m_dh.deserialize(s))
        {
            // dh could not completely been read
            this->removeScheduledBytes();
            this->m_needToRecvData = true;
            return false;
        }
        ...
        {
            LOGERROR(logger,
                     "Illegal message body size occured, buffer size ("
                         << this->m_bufCapacity << " Bytes) exceeded: " << this->m_dh.getMessageSize()
                         << " Bytes in received DataBlock: "
                         << "0x" << std::hex << m_dh.getDataType() << std::dec << " Set to 0.")
        ...
        {
            // message is not yet completely in the message buffer
            if (this->m_bodySize > this->m_bufCapacity - IbeoDataHeader::getHeaderSize())
            {
                // and it will never fit into the buffer.
                LOGERROR(logger,
                         "Expected message size " << IbeoDataHeader::getHeaderSize() + this->m_bodySize
                                                  << " is larger than the available msg buffer size: "
                                                  << this->m_bufCapacity << " Message will be ignored.");
            ...
            {
                // message will fit into the message buffer
    ...
    {
        // no valid data in MsgBufferBase
...
void MsgBufferBase::clearBufferPreserve3Bytes()
{
    ...
    {
        LOGWARNING(logger, nbOfBytesInBuffer - preserve << " Bytes skipped");
    }
}
...
{
    if (this->m_scheduledForDeletionTo == -1)
        return;
    if (this->m_scheduledForDeletionTo >= this->m_bufCapacity || this->m_insAt <= this->m_processedTo)
    {
        clearBufferCompletely();
        return;
    }
