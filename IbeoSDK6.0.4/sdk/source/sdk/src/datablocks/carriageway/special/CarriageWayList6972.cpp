...
CarriageWayList6972::CarriageWayList6972() : SpecializedDataContainer{} {}
...
bool operator==(const CarriageWayList6972& lhs, const CarriageWayList6972& rhs)
{
    ...
    for (std::size_t index = 0; index < lhs.getCarriageWays().size(); ++index)
    {
        ...
        const lanes::CarriageWayIn6972::Ptr& cwLeft{lhs.getCarriageWays()[index]};
        const lanes::CarriageWayIn6972::Ptr& cwRight{rhs.getCarriageWays()[index]};

        if (lanes::CarriageWayIn6972::areSharedPtrObjectsEqual(cwLeft, cwRight) == false)
        {
            return false;
        }
    }
