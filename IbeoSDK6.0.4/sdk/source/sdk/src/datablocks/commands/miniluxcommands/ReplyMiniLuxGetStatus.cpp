...
#include <ibeo/common/sdk/datablocks/commands/miniluxcommands/ReplyMiniLuxGetStatus.hpp>
...
bool ReplyMiniLuxGetStatus::Timestamp::deserialize(std::istream& is)
{
    ...
    ibeo::common::sdk::readLE(is, m_YYYY);
    ibeo::common::sdk::readLE(is, m_MMDD);
    ibeo::common::sdk::readLE(is, m_hhmm);
...
bool ReplyMiniLuxGetStatus::Timestamp::serialize(std::ostream& os) const
{
    ...
    ibeo::common::sdk::writeLE(os, m_YYYY);
    ibeo::common::sdk::writeLE(os, m_MMDD);
    ibeo::common::sdk::writeLE(os, m_hhmm);
...
std::string ReplyMiniLuxGetStatus::Timestamp::toString() const
{
    std::stringstream ss;
    ss << std::setfill('0');
    ss << std::hex << std::setw(4) << getYear() << "-" << std::setw(2) << int(getMonth()) << "-" << std::setw(2)
       << int(getDay()) << " " << std::setw(2) << int(getHour()) << ":" << std::setw(2) << int(getMinute());
    return ss.str();
}
...
ReplyMiniLuxGetStatus::ReplyMiniLuxGetStatus()
  : MiniLuxCommandReply<CommandId::Id::CmdLuxGetStatus>(),
    m_firmwareVersion(0),
    m_fpgaVersion(0),
...
std::string ReplyMiniLuxGetStatus::versionToString(const uint16_t version)
{
    ...
    ss << std::hex << (version >> 12) << '.' << ((version >> 8) & 0xF) << '.' << ((version & 0xF0) >> 4)
       << (version & 0xF);
