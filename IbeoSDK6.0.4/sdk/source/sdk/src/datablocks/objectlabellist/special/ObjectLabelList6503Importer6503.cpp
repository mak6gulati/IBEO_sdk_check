...
using R  = ibeo::common::sdk::RegisteredImporter<C, ibeo::common::sdk::DataTypeId::DataType_ObjectLabel6503>;
...
std::streamsize Importer<C, DataTypeId::DataType_ObjectLabel6503>::getSerializedSize(const DataContainerBase& c) const
{
...
bool Importer<C, DataTypeId::DataType_ObjectLabel6503>::deserialize(std::istream& is,
                                                                    DataContainerBase& c,
                                                                    const IbeoDataHeader& dh) const
{
    ...
    ibeo::common::sdk::readBE(is, container->m_timeOffsetUs);
    ibeo::common::sdk::readBE(is, container->m_timestamp);
    ibeo::common::sdk::readBE(is, container->m_scanNumber);
    ibeo::common::sdk::readBE(is, container->m_scanMidTimestamp);
    ...
    ibeo::common::sdk::readBE(is, nbOfLabels);
    container->m_labels.resize(nbOfLabels);

    for (auto& label : container->getLabels())
    {
        if (!label.deserialize(is))
        {
            break;
        }
    }
