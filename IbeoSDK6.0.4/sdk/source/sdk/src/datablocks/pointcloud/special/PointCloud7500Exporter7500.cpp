...
std::streamsize Exporter<C, DataTypeId::DataType_PointCloud7500>::getSerializedSize(const DataContainerBase& c) const
{
...
bool Exporter<C, DataTypeId::DataType_PointCloud7500>::serialize(std::ostream& os, const DataContainerBase& c) const
{
    const C* container{nullptr};
    try
    {
    ...
    for (std::vector<PointCloudPointIn7500>::const_iterator pointIter = container->m_points.begin();
