...
#include <ibeo/common/sdk/datablocks/vehiclestate/VehicleStateImporter2806.hpp>

//==============================================================================
namespace {
using C  = ibeo::common::sdk::VehicleState;
using R  = ibeo::common::sdk::RegisteredImporter<C, ibeo::common::sdk::DataTypeId::DataType_VehicleStateBasic2806>;
...
std::streamsize
Importer<C, DataTypeId::DataType_VehicleStateBasic2806>::getSerializedSize(const DataContainerBase& c) const
{
    try
    {
...
bool Importer<C, DataTypeId::DataType_VehicleStateBasic2806>::deserialize(std::istream& is,
                                                                          DataContainerBase& c,
                                                                          const IbeoDataHeader& dh) const
{
...

    {
        int32_t posX;
        readBE(is, posX);
        container->setRelativeXPosition(static_cast<double>(posX * 1e-4)); //convert [m * 1e^-4] -> [m]
    }
    {
        int32_t posY;
        readBE(is, posY);
        container->setRelativeYPosition(static_cast<double>(posY * 1e-4)); //convert [m * 1e^-4] -> [m]
    }

    {
        float courseAngle;
        readBE(is, courseAngle);
        container->setCourseAngle(courseAngle * 1e-4F); //convert 0.0001 rad] -> [rad]
    }

    readBE(is, container->m_longitudinalVelocity);
    readBE(is, container->m_yawRate);
    readBE(is, container->m_steeringWheelAngle);
    readBE(is, container->m_crossAcceleration); // (was reserved0)
    readBE(is, container->m_steerAngle);
    readBE(is, container->m_blindPredictionAge); // (was reserved1)
    readBE(is, container->m_vehicleWidth);
    readBE(is, container->m_minTurningCircle); // (was reserved2)
    ...
    readBE(is, container->m_rearAxleToOrigin); // (was reserved3)
