...
Exporter<C, DataTypeId::DataType_PositionWgs84Sequence3510>::getSerializedSize(const DataContainerBase& c) const
{
    ...
    for (const auto& posSeq : container->m_positionSequence)
    {
        result += posSeq.getSerializedSize();
    }
                                                                            ...
                                                                            const DataContainerBase& c) const
{
    ...
    {
        if (!posSeq.serialize(os))
        {
            return false;
        }
    }
    writeBE(os, container->m_timestamp);
    writeBE<ibeo::common::sdk::PositionWgs84Sequence3510::SourceType, uint8_t>(os, container->m_sourceType);
