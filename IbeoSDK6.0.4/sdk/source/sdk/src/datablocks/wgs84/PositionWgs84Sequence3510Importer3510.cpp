...
//==============================================================================
namespace {
using C  = ibeo::common::sdk::PositionWgs84Sequence3510;
...
Importer<C, DataTypeId::DataType_PositionWgs84Sequence3510>::getSerializedSize(const DataContainerBase& c) const
{
    ...
    for (const auto& posSeq : container->getPositionSequence())
    {
        result += posSeq.getSerializedSize(); //Size of PositionSequence
    }
                                                                              ...
                                                                              const IbeoDataHeader& dh) const
{
    ...
    for (auto& posSeq : container->m_positionSequence)
    {
        if (!posSeq.deserialize(is))
        {
            return false;
        }
    }
    ...
    readBE<PositionWgs84Sequence3510::SourceType, uint8_t>(is, container->m_sourceType);
