...
:start
	set dbr=..\include\ibeosdk\DataBlockRegister
	...
	echo #ifdef STATICIBEOSDKLIB>> %dbr%.hpp_test
	for %%f in (datablocks\*.cpp) do call :action %%~nf
	echo #endif // STATICIBEOSDKLIB>> %dbr%.hpp_test
...
:action
	find /c "RegisteredDataBlock" datablocks\%1.cpp
	if %errorlevel% equ 1 goto endloop
	echo %1
