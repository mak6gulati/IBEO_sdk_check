...
class IdcIStreamTestSupport : public TestSupport
{
public:
    using DataBlockInfo = std::pair<IbeoDataHeader, std::shared_ptr<char>>;

public:
    IdcIStreamTestSupport()          = default;
    virtual ~IdcIStreamTestSupport() = default;

    static DataBlockInfo createRandomDatablock();

    static bool writeDataBlock(std::ostream& os, DataBlockInfo& dbi);

protected:
    static IbeoDataHeader fillHeaderRandomly(const uint32_t msgSize);

public:
    bool fillStream(std::ostream& os);
